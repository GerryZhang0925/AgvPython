...
def undistort_image(img, objpts, imgpts):
    ...
    """
    ret, mtx, dist, rvecs, tvecs = cv2.calibrateCamera(objpts, imgpts, cv2.cvtColor(img, cv2.COLOR_RGB2GRAY).shape[::-1], None, None)
...
def detectFromVideo(fname, ld):
    ...
    while (cap.isOpened()):
        ...
        if ret == True:
            ...
            print(n)
            proc_img = ld.process_image(frame)
            #plt.imshow(proc_img)
...
detectFromVideo("project_video.mp4", ld)
#image = ld.process_image(image)
#plt.imshow(image)
#plt.show()
